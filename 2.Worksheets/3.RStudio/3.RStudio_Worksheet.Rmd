...
rm(list=ls())
getwd()
setwd("~/GitHub/QB2021_Graber/2.Worksheets/3.RStudio")
...
```{r}
j <- rnorm(5, mean=8, sd=2)
z <- rnorm(5, mean=25, sd=10)
k <- cbind(z,j)
...
> Answer 1:rnorm generates random numbers that are normally distributed. the first number indicated determines how many random numbers are in the set. Mean= indicates what the mean of the data set should be and sd= the standard deviation of the random number produced. 
...
> Answer 3:CHLA is not correlated with anything else. TN and TP are heavily corrilated,  as are TN and SRP, and TP and SRP. 
...
> Answer 4: Yes, the reuslts are senstive to whether parametic or non-parametric data are used. Non-parametric methods with data that is not normal or with large sample sizes. It does not appear that there is evidence for a false discovery rate, because the correct values are equal to the uncorrected value. It is importatn to ensure that you are not getting false positives (ie. inaccurate results).
...
> Answer 5: As total nitrogen increase zooplankton biomass increases.
...
print(cor4, digits = 3)
```
> Answer 6: CAL, DLIAP, CYCL, BOSM all seem to have their biomass increase in the presence of one another. Also NAUP and CERI and DLUM and DIAP increase in the presence of one another. 
