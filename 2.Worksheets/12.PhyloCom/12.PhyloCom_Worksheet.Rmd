...
rm(list = ls())
getwd()
setwd("~/GitHub/QB2021_Graber/2.Worksheets/12.PhyloCom")
...
plot.phylo (phy, main = "Neighbor Joining Tree", "phylogram", show.tip.label = FALSE, 
            use.eduge.length = FALSE, direction = "right", cex = 0.6, label.offset= 1)
...
exponenet <- round(coefficients(fit)[2], 2)
legend("topleft", legend = paste("scaling exponenet = ", exponenet, sep = ""),
       bty = "n", lw = 2, col = "red")
...
> ***Answer 1a***:  Higher PD indicates that there is higher taxanomica richeness in terms of species (regardless of frequency of occurance of those species). 
> ***Answer 1b***:  Both PD and taxanomic diversity measure species count without considering their abundance. 
> ***Answer 1c***:  If there hasn't been much or any recent divergence the tree lengths would increase causing a higher PD but that would not increase the taxanomic richness value. 
> ***Answer 1d***: PD has an activley lower range than taxanomic richness, this is possibly a product of the extended length of some of the branches we are seeing. 
...
> ***Answer 2a***:For richness we are testing if there is a non-random abundance within sample, for frequency testing if there is a non-random abundance within species, and taxa labels is seeing if the tree structure is equally valid as a random tree.   
> ***Answer 2b***:  When the out put is greater than expected the site is more diverse than the null hypothesis. This true for pond 2 when running null richness, pond 2 when running null taxa labels, and pond 1 when running null frequency. Freqency produced a different results which shifts our view of community structure than the phylogenetic tree impacting the preception of species richness. 
...
rownames(NRI) <- row.names(ses.mntd)
colnames(NRI) <- "NTI"
```
...
> ***Answer 3a***:  Using a calculation of the avaerage branch lenght between taxa in a sample I am testing if there is clustering or if there is more variation than expected (overdispersion) in the phylogeny. 
> ***Answer 3b***:  This is also testing for clustering and overdispersion in the phylogeny, but this time using the average distance between each taxa and their closest neighbor. 
> ***Answer 3c***:  NRI shows that everything is overdispersed (all results are negative) while NTI shows a lot of overdispersion the degree to which is lower and there are some small positive values showing that there is also some clustering within this result 
> ***Answer 3d***:  There is a lot more clustered overall, each still producing some negative/overdispersion values. 
...
> ***Answer 4a***: Mean pair distance is calculating the similariy between two samples as a value for the mean phylogenetic distance between taxa pairs. Unifrac is calculating the similarity between two samples be considering the sum of unshared branch lengths by the total # of branch lenghts. The difference between these calculations is that MPD is considering the approx. amount of time between branches spliting while unifrac is considering branch lengths which are more a reprentation of the amount of change in that species and less about time itself. 
> ***Answer 4b***: MPD has far less diversity of results (primarily all around 0.3) while unifrac has a diversity of outputs ranging from 0.2 to 0.45. 
> ***Answer 4c***: Unifrac is produced as more of a ratio for each two samples while MPD is calculated as a mean phylogentic distance essentially showing if two species are closely related. 
...
> ***Answer 5***: What was already a reletively low PCoA 1 and 2 percentage output became even smaller upon using the main pair distance data. This shows that phylogentic data is rather important in this system while taxanomic data is not. 
...
> ***Answer 6***: The phyloegenetic patterns seem to be most greatly impacted by elevation, pH, temperature, chlorophyll alpha presence, depth, and the conductivity of the water. 
...
> They could be useful to consider when certain pollen presentation mechansims evolved and what conditions did or at least my have constrained or pushed towards their evoltuion. I am comparing pollen pump mechanisms between various lobelia species and potentially expanding beyond that so considering phylogentic structure is incredibly valuable. 