...
rm(list=ls()) 
getwd() 
setwd("~/GitHub/QB2021_Graber/2.Worksheets/7.DataWrangling")
```
...
>0.5951182, is the Z value. 

```{r}

com2 <- sim_poisson_community(s_pool = 25, n_sim = 1000, sad_type = "lnorm", 
        sad_coef = list("meanlog" = 2, "sdlog" = 1))

quad_areas <- seq(0.005, 0.5, 0.005)
dfs <- vector("list", length(quad_areas))
for (qa in 1:length(quad_areas) ) {
  sampl <- sample_quadrats(com2, 5, quadrat_area = quad_areas[qa],  plot = FALSE, avoid_overlap=FALSE)
     sampl_df <- sampl$spec_dat %>%
...
>0.6174736, is the z value. 

```{r}
comtom <- sim_thomas_community(s_pool = 25, n_sim = 1000, sad_type = "lnorm", 
        sad_coef = list("meanlog" = 2, "sdlog" = 1))
quad_areas <- seq(0.005, 0.5, 0.005)
dfs2 <- vector("list", length(quad_areas))
for (qa in 1:length(quad_areas) ) {
  sampl2 <- sample_quadrats(comtom, 5, quadrat_area = quad_areas[qa],  plot = FALSE, avoid_overlap=FALSE)
     sampl2_df <- sampl2$spec_dat %>%
       mutate(richness =rowSums(.>1))  %>%    
       mutate(area = quad_areas[qa]) %>%
       select(area, richness)
      dfs2[[qa]]<- sampl2_df
}
all_samps2_nonzero <- bind_rows(dfs2) %>%
  filter(richness >0) %>%
  mutate(ln_richness = log10(richness)) %>%
  mutate(ln_area = log10(area))
all_samps2_nonzero <- bind_rows(dfs2) %>%
  ...
  mutate(ln_richness = log10(richness)) %>%
  mutate(ln_area = log10(area)) 
SA_plot <- ggplot(all_samps2_nonzero, aes(x= ln_area, y=ln_richness))+ 
  geom_point() +
  stat_smooth(method='lm')
SA_plot
linreg2 <- lm(ln_richness ~ ln_area, all_samps2_nonzero) 
print(linreg2$coefficients[['ln_area']])
```
...
Describe. 
>There is generally more clustering in sim_thomas which is expected as it is simulating a more patching distribution. For a more elaborate simulation x and y ranges can be implemeneted. There could be consideration of the environment/geography, changes over time, and account for metacommunity structure. 
